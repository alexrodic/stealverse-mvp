...
The final archive is created from the project root and excludes `.git/`, `node_modules/`, `dist/`, and any legacy `todo/` directory. Required publish files are included: `package-lock.json`, working `preview` script, Vitest tests, `.github/workflows/ci.yml`, `vercel.json`, `README.md`, `docs/`, and this handoff.
...
- **GitHub:** not verified or published. No repository checkout or supported `commit_and_push` tool was available, and no commit SHA is claimed.
